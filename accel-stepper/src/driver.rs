...
impl Driver {
    ...
    pub fn set_max_speed(&mut self, steps_per_second: f32) {
        ...
        self.min_step_size = 1000000.0 / steps_per_second;
    }
    ...
    pub fn set_speed(&mut self, speed: f32) {
        ...
        if speed == 0.0 || !speed.is_finite() {
            ...
            self.step_interval = (1000000.0 / speed).abs().round() as u64;
        }
    ...
    fn compute_new_speed(&mut self) {
        ...
        self.step_counter += 1;
        self.step_interval = self.last_step_size.round() as u64;
        self.speed = 1000000.0 / self.last_step_size;
        ...
        D: Device,
    {
        ...
        if now.as_micros() - self.last_step_time >= self.step_interval.into() {
            ...
            let ctx = StepContext {
                position: if self.distance_to_go() > 0 { 1 } else { 0 },
                step_time: now,
            };
            device.step(&ctx)?;

            self.current_position = new_position;
            self.last_step_time = now.as_micros(); // Caution: does not account for costs in step()
